...
type Processor interface {
	Score(*metrics.NodeFullMetric) (float64, float64)
	ExtraWeight(int32)
	N(*metrics.NodeInfoRecord)
	Even(*metrics.NodeInfoRecord)
	Variance(*metrics.NodeInfoRecord)
}
...
func (sp *ScoreProcessor) Score(nfm *metrics.NodeFullMetric, postFuncs ...func(ProcessorType, float64, float64)) (float64, float64) {
...
func (cp *CPUProcessor) Score(nfm *metrics.NodeFullMetric) (float64, float64) {
...
func (*CPUProcessor) N(record *metrics.NodeInfoRecord) {
...
func (*CPUProcessor) Even(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (*CPUProcessor) Variance(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (mp *MemoryProcessor) Score(nfm *metrics.NodeFullMetric) (float64, float64) {
...
func (*MemoryProcessor) N(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (*MemoryProcessor) Even(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (*MemoryProcessor) Variance(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (dup *DiskUsageProcessor) ExtraWeight(w int32) {
	atomic.StoreInt32(&dup.extraWeight, w)
}

func (dup *DiskUsageProcessor) Score(nfm *metrics.NodeFullMetric) (float64, float64) {
...
func (*DiskUsageProcessor) N(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (*DiskUsageProcessor) Even(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (*DiskUsageProcessor) Variance(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (np *NetworkProcessor) Score(nfm *metrics.NodeFullMetric) (float64, float64) {
...
func (*NetworkProcessor) N(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (*NetworkProcessor) Even(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func (*NetworkProcessor) Variance(record *metrics.NodeInfoRecord) {
	idx := len(record.Metrics) - 1
	if len(record.Metrics) == 0 {
		return
	}
...
func calWeight(ms metrics.MetricStatistics) float64 {
